...
// https://codefellows.github.io/code-301-guide/curriculum/class-08/challenges/DEMO.html
...
// number of children in each house (asc). 
// If a house has the same number of children, sort alphabetically by house name.
...
const containsWorld = (input) => {
  // Solution code here...
  const testWorld = /(world)/g
  return testWorld.test(input)
};
...
const isCapitalized = (str) => {
  // Solution code here...
  const containsCapital = /[A-Z][a-z]+/g;
  if (str.match(containsCapital))
    return str.match(containsCapital)
  else
    return []
};
